...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        try:
            script_path = os.path.join(os.path.dirname(__file__), "..", "jobs", "ehr_inventory.py")
            result = subprocess.run(
                [sys.executable, script_path],
                capture_output=True, text=True, check=True, timeout=280,
                env=os.environ.copy(),
            )
            ...
            self.wfile.write(json.dumps({
                "status": "timeout", "error": "Timeout expired",
                "stdout": stdout_str[-12000:], "stderr": stderr_str[-12000:],
            }).encode())
