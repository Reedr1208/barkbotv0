...
class FactProfileExtraction(BaseModel):
    ...
    highlights: List[str] = Field(description="1-3 short, punchy, adopter-appeal phrases (1-4 words each) highlighting the dog's most desirable qualities. Think 'what makes someone stop scrolling'. Examples: 'Cuddle Bug', 'Gentle Giant', 'Total Sweetheart', 'Loyal Companion'. NEVER include sex, age, weight, breed, color, or any stat already displayed elsewhere.")
    important_facts_jsonb: List[str] = Field(description="Array of 1-4 word phrases focused on unique personality traits, special needs, behavioral observations, or living situation details that help an adopter understand daily life with this dog. Examples: 'Loves Fetch', 'Needs Slow Intros', 'In Foster Care', 'Leash Trained', 'Dog Selective'. NEVER include sex, age, weight, breed, color, or physical stats.")
