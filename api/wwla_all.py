...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        logging.info("Starting up scrape_wwla handler...")
        ...
        if cron_secret:
            ...
            if auth_header != expected:
                logging.warning("Unauthorized access attempt to scrape_wwla.")
                self.send_response(401)
                self.end_headers()
                self.wfile.write(b"Unauthorized")
                return
        
        target_script = Path(__file__).resolve().parent.parent / "jobs" / "wwla_all.py"
