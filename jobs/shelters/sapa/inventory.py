"""
SAPA (San Antonio Pets Alive) — Inventory Scraper

Scrapes adoptable dog listings from sanantoniopetsalive.org using Playwright
(Shelterluv-powered embed). This job requires Playwright and must run
via GitHub Actions (not Vercel crons).

Adapted from the PHP inventory scraper which uses the same Shelterluv
embed technology.
"""
...
# CONFIG
# =============================================================================
START_URL = "https://sanantoniopetsalive.org/adopt/"
...
# Shelterluv prefix for this shelter is SAPT; our shelter_id is SAPA
...
SKIP_IMAGE_PATTERNS = re.compile(
    r"(logo|favicon|badge|charity|guidestar|bbb|amazon|facebook|instagram|youtube|tiktok|pixel|tracking|yarn-ball|dog-butt|arrow|splat|cat-pair)",
    re.I,
)
...
# Reuse the same JS extraction logic from HHS/PHP — it's generic Shelterluv tile extraction
EXTRACT_JS = r"""
() => {
  function absUrl(u) {
    if (!u) return '';
