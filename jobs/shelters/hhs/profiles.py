"""
HHS (Houston Humane Society) — Profile Scraper

Fetches individual dog profile pages from Shelterluv embeds and extracts
structured fields + bio text from the :animal JSON attribute.
...
def fetch_record(url: str, target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Filter out non-dogs
    species = (animal_data.get("species") or "").strip().lower()
    ...
    # Build bio — include all available structured data
    bio_parts = []
    if breed:
        full_breed = breed
        if secondary_breed:
